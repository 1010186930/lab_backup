...
u8 k1_event(u8 curfiles)
{
    ...
    memset(vocalname,'\0',sizeof(vocalname));
    dir=(DIR*)mymalloc(SRAMIN,sizeof(DIR));
    finfo =(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));
    finfo->lfsize=_MAX_LFN*2+1;
    finfo->lfname= mymalloc(SRAMIN,finfo->lfsize);
    ...
    if(res==0)
    {
        ...
        if(demo==1)
        {
            ...
            strncpy(midname,filename[0]+2,strlen(filename[0])-2);
            if(strstr(midname,"Lesson"))
            {
                strcpy(vocalname,"[g2][h2][s3]");
                strcat(vocalname,midname);
                SPeech(vocalname);
            }
            ...
            else
            {
                if(!strcmp(midname,"第一课 标调基本规则"))
                    SPeech("[i1][n2][s3]第一课 标掉基本规则");
                else
                {
                    strcpy(vocalname,"[i1][n2][s3]");
                    delay_ms(10);
                    strcat(vocalname,midname);
                    SPeech(vocalname);
                }
    ...
    else if(demo==0)
    {
        ...
        if(strstr((const char*)path,"Lesson"))
            SPeech("[g2][h2][s3]");
        else
            SPeech("[i1][n2][s3]");
        delay_ms(10);
...
//		printf("创建书签结果%d\r\n",res);
    ...
    if(res==0)
    {
        ...
        else
        {
            ...
            if(strstr(midname,"uā  uāi  uī  uō  üē  ér"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                strcat(vocalname,"第十一课 韵母  uā  uāi  uī  uō  vē  ér");
                SPeech(vocalname);
            }
            else if(strstr(midname,"üān"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                strcat(vocalname,"第十五课 韵母  vān   ǖn   iōng");
                SPeech(vocalname);
            }
            else if(!strcmp(midname,"第一课 标调基本规则"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                strcat(vocalname,"第一课 标掉基本规则");
                SPeech(vocalname);
            }
            else if(!strcmp(midname,"标调规则教程"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                strcat(vocalname,"标掉规则教程");
                SPeech(vocalname);
            }
            else if(!strcmp(midname,"通用盲文标调规则教程"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                strcat(vocalname,"通用盲文标掉规则教程");
                SPeech(vocalname);
            }
            else
            {
...
//		else if(strstr((const char*)buffer1,"üē")&&strstr((const char*)buffer1,"uā"))
//		else if(!strcmp(buffer1,"韵母 uā  uāi  uī  uō  üē  ér   "))
//		{
//			printf("=====================OK=====================");
//			SPeech("韵母 uā uāi uī uō vē ér");
//		}
...
//	}
    ...
    if(res==FR_OK)
    {
        ...
        if(strstr(bkmidname,"Lesson"))
        {
            strcpy(vocalname,"[g2][h2][s3]");
            strcat(vocalname,bkmidname);
            SPeech(vocalname);
        }
        else
        {
            if(strstr(bkmidname,"üē"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                q = strchr(bkmidname, ' ');
                *(q+1) = '\0';
                strcpy(speechname, bkmidname);//
                strcat(speechname, "第十一课 韵母uā uāi uī uō vē ér");
                strcat(vocalname,speechname);
                SPeech(vocalname);
            }
            else if(strstr(bkmidname,"üān"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                q=strchr(bkmidname,' ');
                *(q+1) = '\0';
                strcpy(speechname, bkmidname);//
                strcat(speechname,"第十五课 韵母 vān ǖn iōng");
                strcat(vocalname,speechname);
                SPeech(vocalname);
            }
            else if(strstr(bkmidname,"标调基本规则"))
            {
                strcpy(vocalname,"[i1][n2][s3]");
                q=strchr(bkmidname,' ');
                *(q+1) = '\0';
                strcpy(speechname, bkmidname);//
                strcat(speechname,"第一课 标掉基本规则");
                strcat(vocalname,speechname);
                SPeech(vocalname);
            }
            else
...
void move_Bookmarkcursor_Next(u16 n)//选择下一个书签
{
    ...
    if(strstr(bkmidname,"Lesson"))
    {
        ...
        strcpy(vocalname,"[i0][g2][h2][s3]");
        strcat(vocalname,bkmidname);
        SPeech(vocalname);
    }
    else
    {
        if(strstr(bkmidname,"uā  uāi  uī  uō  üē  ér"))
        {
            strcpy(vocalname,"[i1][g1][n2][s3]");
            q = strchr(bkmidname, ' ');
            *(q+1) = '\0';
            strcpy(speechname, bkmidname);//
            strcat(speechname, "第十一课 韵母 uā uāi uī uō vē ér");
            strcat(vocalname,speechname);
            SPeech(vocalname);
        }
        else if(strstr(bkmidname,"üān"))
