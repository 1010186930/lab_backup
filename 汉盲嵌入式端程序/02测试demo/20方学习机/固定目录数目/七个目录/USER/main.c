...
u8 menu;
u8 key0=0,key1=0,key2=0,key3=0,key4=0,key5=0,key6=0,key7=0;
	...
	switch(menu)
	{
	  case 0:
		...
		case 6:
			key6=(key6+1)%n6;
		highlight(key6);	
									strncpy(midname,name[key6]+2,strlen(name[key6])-2);
		SPeech(midname);			
						memset(midname,'\0',sizeof(midname));
				//		SPeech(name[key4]);
		   delay_ms(1);
		...
		case 7:
			key7=(key7+1)%n7;
		highlight(key7);	
									strncpy(midname,name[key7]+2,strlen(name[key7])-2);
		SPeech(midname);			
						memset(midname,'\0',sizeof(midname));
				//		SPeech(name[key4]);
		   delay_ms(1);
...
delay_ms(1);				
		...
		case 6:
			key6=(key6+1)%n6;
		highlight(key6);	memset(midname,'\0',sizeof(midname));
		strncpy(midname,name[key6]+2,strlen(name[key6])-2);
		SPeech(midname);					
		...
		case 7:
			key5=(key7+1)%n7;
		highlight(key7);	memset(midname,'\0',sizeof(midname));
		strncpy(midname,name[key7]+2,strlen(name[key7])-2);
		SPeech(midname);					
...
/*if(key_v==4) e3_event(&menu);//书签
		...
		if(key_v==3) 
			e5_event(&menu);//上一页，Key1
	else if(key_v==1) 
		e1_event(&menu);//OK，Key0
	else if(key_v==2) 
		e4_event(&menu);//下一页，Key2
	else if(key_v==5)  
		e2_event(&menu);//上一级，Key4
	else if(key_v==4)   
		e6_event(&menu);//下一页目录端，Key3
	if(countdown==60000)
		{
			dis(0);
...
void UIfun(u8 state)
{
	...
	//			SPeech(name[key2]);
		...
		break;
		case 9:
			display("0:/02一年级上课文（不带音调）",key2,n2);
		break;
		case 10:
			display("0:/03一年级上课文（带音调）",key3,n3);
		break;
		case 11:
			display("0:/04一年级下课文（不带音调）",key4,n4);
		break;
		case 12:
			display("0:/05一年级下课文（带音调）",key5,n5);
		break;
		case 13:
			display("0:/06古诗（不带音调）",key6,n6);
		break;
		case 14:
			display("0:/07古诗（带音调）",key7,n7);
		break;
	}
}
