...
void selectSort(char(*pt)[50], int n);//书签排序函数
  ...
  pathp[0]=0;
	...
	if(res==0)
	{
	  while(1)
		...
		if(!strcmp(midname,"第十一课 uā uāi uī uō üē ér"))
		{
			SPeech("第十一课 uā uāi uī uō vē ér");
		}
		else if(!strcmp(midname,"第十五课 üān ǖn iōng"))
		{
			SPeech("第十五课 vān ǖn iōng");
		}	
		else
			SPeech(midname);
		curplace=k;
...
void play()
{
	...
	else if(strstr((const char*)path,"11第十一课 uā uāi uī uō üē ér"))
	{
		Show_Str(18,80,800,32,(u8*)buffer1,32,1);
...
//		}	
	...
	else if(strstr((const char*)path,"15第十五课 üān ǖn iōng"))
	{
		Show_Str(18,80,800,32,(u8*)buffer1,32,1);
...
//	}
	...
	if(res==FR_OK)
	{
		...
		if(strstr(bkmidname,"第十一课 uā uāi uī uō üē ér"))
		{
			q = strchr(bkmidname, ' ');
			*(q+1) = '\0';
			strcpy(speechname, bkmidname);//
			strcat(speechname, "第十一课 uā uāi uī uō vē ér");
			SPeech(speechname);
		}
		else if(strstr(bkmidname,"第十五课 üān ǖn iōng"))
		{
			q=strchr(bkmidname,' ');
			*(q+1) = '\0';
			strcpy(speechname, bkmidname);//
			strcat(speechname,"第十五课 vān ǖn iōng");
			SPeech(speechname);
		}	
		else	
		SPeech(bkmidname);
