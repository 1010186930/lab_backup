...
void ASCIItoBraille(char *path);//生成ascii对应的点序文件
...
while(1)
	{
		...
		if(key_v==1&&demo==1)
		{
		  ...
		  n_files=k1_event(curplace);
			...
			k5_event(flag);//文本端上一页
		else if(key_v==2&&demo==0)
			 k4_event(flag);//文本端下一页
//		else if(key_v==4)
//		  k4_event();
    else if(key_v==5)
...
u8 k1_event(u8 curfiles)
{
  ...
  strcat((char*)pathp,"/");
	...
	if(res==0)
	{
		while(1)
		{
			...
			else 
			{
				...
				{
					if(strcmp("atob.txt",(const char*)fn))
						flag=1;//没有atob.txt
					else 
					{
						flag=2;//有atob.txt
	...
	else if(demo==0&&flag==1)
	{
		
		ASCIItoBraille((char*)path);
		flag=2;
	}
	if(demo==0&&flag==2)
	{
...
void ASCIItoBraille(char *path)
{
	...
	transerror=(FIL*)mymalloc(SRAMIN,sizeof(FIL));
	ptxt=(char*)mymalloc(SRAMIN,511);
	pdata=(char*)mymalloc(SRAMIN,511);
	prror=(char*)mymalloc(SRAMIN,511);
	ptxt[0]=pdata[0]=prror[0]=0;
	mymemset(ptxt,0,511);
	mymemset(pdata,0,511);
	mymemset(prror,0,511);
	strcat((char*)ptxt,(char*)path);
	strcat((char*)pdata,(char*)path);
	strcat((char*)prror,(char*)path);
	strcat((char*)ptxt,"/text.txt"); 
	strcat((char*)pdata,"/atob.txt");
	strcat((char*)prror,"/error.txt");
	res=f_opendir(dir,"0:");
	ret=f_open(filasc,(char*)ptxt,FA_READ);
	red=f_open(fildata,(char*)pdata,FA_CREATE_NEW|FA_WRITE);
	rep=f_open(transerror,(char*)prror,FA_CREATE_NEW|FA_WRITE);
	f_close(fildata);
	f_close(transerror);
	Show_Str(60,110,144,32,"正在翻译:",32,0);
	SPeech("开始翻译");
	delay_ms(100);
	cntflag=1;
	if(ret==FR_OK)
	{
		f_open(transerror,(char*)prror,FA_WRITE);
		f_open(fildata,(char*)pdata,FA_WRITE);
		while(f_tell(filasc)<f_size(filasc))
		{
			res = f_read(filasc,&ascii,sizeof(ascii),&br);
			printf("%c\r\n",ascii);
			if((int)ascii<32||(int)ascii>122||(int)ascii==96)
			{
				f_write(transerror,&ascii,strlen(&ascii),&ew);
				printf("未 识 别 的 字 符：%c\r\n",ascii);
			}
			else
			{
