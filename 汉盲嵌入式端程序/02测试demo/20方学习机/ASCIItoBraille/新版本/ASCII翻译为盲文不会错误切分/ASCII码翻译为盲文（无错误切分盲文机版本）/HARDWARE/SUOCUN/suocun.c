...
//suo1-15如果是0，不管输出是什么，都被锁定 GPIOC是锁存器的输入引脚，15个锁存器的输入引脚连在一起
//GPIOF和GPIOG的前3组是锁存器的锁定引脚LE
//D1~D8分别对应PC0,PC1,PC2,PC3,PC4,PC5,PC6,PC7
void SUOCUN_init(void)
{
	...
	void DX_ShowBraille(char tbtmp[10][13]){
	 //1.字符串重构，并转整型
	 /*配合IO的有限没有120个IO口，采用15个锁存器扩展IO口，15*8=120，所以数据必须后期处理8位为一组.	
		同时考虑到盲文点字序号和stm32的IO输出（硬件连接锁存器D0~D8低到高）顺序相反，所以必须翻转后输出达到正确控制线圈而显示盲文点字*/
	//对字符数组btmp进行重组为8位有效字符一组，并转换为整型值作为锁存器的数据信号
       u8 count_s;//计数器
	   u16 dx[15];		//
	 char gdtmp[9];
	 //char stbtmp[120];//过渡数组gd
	 char ts[2][6];
	 int i,j,a,b,m,n,x1,x2,y,l,k;
...
void DX_FCBraille(char tbtmp[20][7])
	{	
		u8 count_s,i=0;
		u16 dx[15];
		char gdtmp[9]; 
