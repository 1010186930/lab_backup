...
*********************************************************************************************************
*
*	模块名称 : 英文盲文机的RGB6CD驱动程序
*	文件名称 : ltdc.c
*	版    本 : V1.0
*	说    明 : STM32F429 内部LTDC接口的硬件配置程序。
*	修改记录 :
*		版本号  日期       作者    说明
*		V1.0    2021-07-07 Kevin 配置硬件接口、LTDC时序和DMA2D
*
*********************************************************************************************************
*/ 
...
u32 *ltdc_framebuf[2];					//LTDC LCD帧缓存数组指针,必须指向对应大小的内存区域，该模块只使用了第一层
...
void LTDC_Layer_Parameter_Config(u8 layerx,u32 bufaddr,u8 pixformat,u8 alpha,u8 alpha0,u8 bfac1,u8 bfac2,u32 bkcolor)
{
	...
	pLayerCfg.BlendingFactor1=0x00000400U;    //设置层混合系数
	pLayerCfg.BlendingFactor2=0x00000005U;	//设置层混合系数
...
u16 LTDC_PanelID_Read(void)
{
    ...
    __HAL_RCC_GPIOJ_CLK_ENABLE();       //使能GPIOG时钟
...
void LCDRST_Init(void)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_4;               //PH4推挽输出，控制复位
...
{   
	...
	}else if(lcdid==0X7084)
	{
		...
		lcdltdc.vfp=22;				    //垂直前廊
        LTDC_Clk_Set(66,2,RCC_PLLSAIDIVR_2); //设置像素时钟 33M(如果开双显,需要降低DCLK到:18.75Mhz  300/4/4,才会比较好)
...
/* GPIOs Configuration */
		...
		+                       LCD pins assignment                                   +
		+------------------------+-----------------------+----------------------------+
		|  LCD429_TFT R0 <->  NULL  |  LCD429_TFT G0 <->  NULL |  LCD429_TFT B0 <->  NULL      |
		|  LCD429_TFT R1 <->  NULL  |  LCD429_TFT G1 <->  NULL |  LCD429_TFT B1 <->  NULL      |
		|  LCD429_TFT R2 <->  NULL  |  LCD429_TFT G2 <-> PJ.09 |  LCD429_TFT B2 <->  NULL      |
		...
		-------------------------------------------------------------------------------
		|  LCD429_TFT HSYNC <-> PI.12  | LCDTFT VSYNC <->  PI.13 | LCDTFT BL  <-> PB.09
		|  LCD429_TFT CLK   <-> PI.14  | LCD429_TFT DE<->  PK.07 | LCDTFT RST <-> PH.04
		-----------------------------------------------------
		*/	
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* hltdc)
{
    ...
    //初始化PB9，背光引脚
    GPIO_Initure.Pin=GPIO_PIN_9;                //PB9推挽输出，控制背光
    GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;      //推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;              //上拉        
    GPIO_Initure.Speed=GPIO_SPEED_HIGH;         //高速
