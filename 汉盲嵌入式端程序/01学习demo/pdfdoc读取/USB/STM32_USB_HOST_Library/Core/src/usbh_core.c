               ...
               USBH_Usr_cb_TypeDef *usr_cb)
{
  ...
  /* Hardware Init 硬件初始化与pdev无关*/
  USB_OTG_BSP_Init(pdev);  
  
  /* configure GPIO pin used for switching VBUS power 没用到*/
  USB_OTG_BSP_ConfigVBUS(0);  
  
  
  /* Host de-initializations pdev用于解除通道*/
  ...
  /* Upon Init call usr call back 打印USBH_USR_Init的三句话*/
  phost->usr_cb->Init();
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  if (((HCD_IsDeviceConnected(pdev) == 0)|| (HCD_IsPortEnabled(pdev) == 0))&& (phost->gState != HOST_IDLE)) /*disconnected or unanble or not HOST_IDLE*/
  {
