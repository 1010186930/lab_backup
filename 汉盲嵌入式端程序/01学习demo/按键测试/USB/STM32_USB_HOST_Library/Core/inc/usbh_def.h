...
{
  uint8_t   bLength;/*0x12 描述符的字节长度*/
  uint8_t   bDescriptorType;/*0x01 设备描述符类型*/
  uint16_t  bcdUSB;        /* USB Specification Number which device complies too */
  uint8_t   bDeviceClass;
  uint8_t   bDeviceSubClass; 
  uint8_t   bDeviceProtocol;/*对于U盘而言，位456要固定写成0，因为是接口类设备*/
...
typedef struct _ConfigurationDescriptor
{
  ...
  uint8_t   bDescriptorType;/*0x02配置描述符类型*/
...
{
  uint8_t bLength;/*0x09*/
  uint8_t bDescriptorType;/*0x04接口描述符类型*/
  uint8_t bInterfaceNumber;
  uint8_t bAlternateSetting;    /* Value used to select alternative setting */
  uint8_t bNumEndpoints;        /* Number of Endpoints used for this interface */
  uint8_t bInterfaceClass;      /* Class Code (Assigned by USB Org) MASS STORAGE Class固定为0x08*/
  uint8_t bInterfaceSubClass;   /* Subclass Code (Assigned by USB Org) 0x06表示SCSI命令集 对于软盘CD-ROM 使用UFI命令集*/
  uint8_t bInterfaceProtocol;   /* Protocol Code 0x50 支持BULK-ONLY传输*/
...
{
  uint8_t   bLength;/*0x07端点描述符*/
  uint8_t   bDescriptorType;/*0x05端点描述符类型*/
  uint8_t   bEndpointAddress;   /* indicates what endpoint this descriptor is describing 有上传0x8?和下传0x0?*/
  uint8_t   bmAttributes;       /* specifies the transfer type. 0x02*/
  uint16_t  wMaxPacketSize;    /* Maximum Packet Size this endpoint is capable of sending or receiving 0x08,0x10,0x20,0x40*/  
