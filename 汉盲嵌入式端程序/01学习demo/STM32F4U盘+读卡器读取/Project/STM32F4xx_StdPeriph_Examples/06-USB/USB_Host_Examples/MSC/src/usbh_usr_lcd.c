...
//    LCD_UsrLog("> Disk capacity : %d Bytes\n", (USBH_MSC_Param.MSCapacity) * \
    ...
    LCD_UsrLog("> Disk capacity : %d MB\n", USBH_MSC_Param.MSCapacity>>11);
		LCD_UsrLog("> Disk PageLength : %d Bytes\n", USBH_MSC_Param.MSPageLength);
  ...
  if (res == FR_OK) {
    while(HCD_IsDeviceConnected(&USB_OTG_Core)) 
    {
      ...
      if (line_idx > 9) {
        ...
        /*Key B3 in polling*/																			//Waiting UserKey Press
        while((HCD_IsDeviceConnected(&USB_OTG_Core)) && \
          (STM_EVAL_PBGetState (BUTTON_USER) == RESET)) {
          Toggle_Leds();
      ...
      if ((fno.fattrib & AM_MASK) == AM_DIR) {			//It is a File
        strcat(tmp, "\n"); 
        LCD_UsrLog((void *)tmp);
      } else {																			//It is a Folder
        strcat(tmp, "\n"); 
...
static void Show_Image(void)
{
  ...
  FRESULT res; 
	...
	LCD_Window(0,0,240,320);
	
//  LCD_SetDisplayWindow(240,320,0,0);
