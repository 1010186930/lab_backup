/**
  ...
  * @version V1.0.2
  * @date    26-Jan-2013
  * @brief   This file includes the LCD driver for LCD_ILI9320 and LCD_ILI932 
              and LCD_ILI9328 Liquid Crystal Display Modules of STM32F4 Discovery
  ******************************************************************************
 */   
/* Includes ------------------------------------------------------------------*/
...
/** @Note  Define LCD REG and RAM address. These values depend on the connection of 
           FSMC_NEx and FSMC_Ax. 
           In this program, FSMC_NE1 and FSMC_A16 are used 
   */    
#define DISCOVERY_LCD_REG  *(uint16_t *)(0x60000000)
#define DISCOVERY_LCD_RAM  *(uint16_t *)(0x60020000)

/** @defgroup STM32F4_DISCOVERY_LCD_Private_Defines
  * @{
  */
...
/* Configure the FSMC Parallel interface -------------------------------------*/
  ...
  else if(lcdid == 0x9325 || lcdid == 0x9328) /* Check if the LCD is ILI9325 Controller */
  {
    /* Start Initial Sequence ------------------------------------------------*/
    LCD_WriteReg(LCD_REG_0, 0x0001); /* Start internal OSC. */
    LCD_WriteReg(LCD_REG_1, 0x0100); /* Set SS and SM bit */
...
void LCD_WriteReg(uint8_t LCD_Reg, uint16_t LCD_RegValue)
{
  ...
  DISCOVERY_LCD_REG = LCD_Reg;
  /* Write 16-bit Reg */
  DISCOVERY_LCD_RAM = LCD_RegValue;
}
...
{
  /* Write 16-bit Index (then Read Reg) */
  DISCOVERY_LCD_REG = LCD_REG_34; /* Select GRAM Reg */
  /* Read 16-bit Reg */
  return DISCOVERY_LCD_RAM;
}
...
void LCD_Window(u16 x1, u16 y1, u16 x2, u16 y2)
{
	  ...
	  LCD_WriteReg(0x20, x1);		//设置点座标
  	LCD_WriteReg(0x21, y1);
	
		DISCOVERY_LCD_REG=0x22;    		    //开始写入GRAM
}
/**
  * @}
  */ 
