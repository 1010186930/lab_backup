...
#define FONTSECSIZE	 	1600
//字库存放起始地址 
#define FONTINFOADDR 	1024*1024*10					//WarShip STM32F103 V3是从12M地址以后开始存放字库
														//前面12M被fatfs占用了.
...
//一个BLOCK可以分成16个扇区，每个扇区4096字节（4KB）
//4个字库+其他=6303532字节，约占1539个扇区

_font_info ftinfo;

//字库存放在磁盘中的路径
u8*const GBK32_PATH="/SYSTEM/FONT/GBK32.FON";		//GBK24的存放位置
 	...
 	if(rval==0)	 
	{
		switch(fx)
		{
			...
			case 3:
				...
				ftinfo.gkb32size=fftemp->fsize;					//GBK24字库大小
...
{	
	...
	{  
	 ...
	 LCD_ShowString(x,y,240,320,size,"Updating GBK32.BIN  ");
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK32_PATH); 
		res=updata_fontx(x+20*size/2,y,size,pname,4);	//更新GBK24.FON
