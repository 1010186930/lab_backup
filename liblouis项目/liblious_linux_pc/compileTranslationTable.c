...
getAChar(FileInfo *nested) {
	 ...
	 * 16- or 32-bit unsigned integers 读取大端、小端或ASCII 8文件，并将其转换为
* 16或32位无符号整数*/
	int ch1 = 0, ch2 = 0;
	widechar character;
...
_lou_getALine(FileInfo *nested) {
	...
	/* Read a line of widechar's from an input file 从输入文件中读取一行宽字符*/
	int ch;
	int pch = 0;
	nested->linelen = 0;
...
static inline int
atTokenDelimiter(FileInfo *nested) {/*在标记分隔符处*/
...
getToken(FileInfo *nested, CharsString *result, const char *description, int *lastToken) {
	 ...
	 * 查找下一个连续的非空白字符字符串。如果这是该行的最后一个标记，则返回2而不是1。*/
	...
	while (!atEndOfLine(nested) && !atTokenDelimiter(nested)) {
		...
		if (result->length >= maxlen) {/*长度不能太长*/
...
static int
allocateSpaceInDisplayTable(FileInfo *nested, TranslationTableOffset *offset, int count,/*函数在显示表中分配空间*/
		DisplayTableHeader **table) {
	/* allocate memory for table and expand previously allocated memory if necessary  为表分配内存，必要时扩展以前分配的内存*/
	int spaceNeeded = ((count + OFFSETSIZE - 1) / OFFSETSIZE) * OFFSETSIZE;
	TranslationTableOffset newSize = (*table)->bytesUsed + spaceNeeded;
	TranslationTableOffset size = (*table)->tableSize;
	if (newSize > size) {
...
static int
allocateTranslationTable(FileInfo *nested, TranslationTableHeader **table) {/*分配翻译表*/
	/* Allocate memory for the table and a guess on the number of rules 为表分配内存，并猜测规则的数量*/
	const TranslationTableOffset startSize = 2 * sizeof(**table);
	if (*table) return 1;
	TranslationTableOffset bytesUsed =
			sizeof(**table) + OFFSETSIZE; /* So no offset is ever zero 所以没有偏移量是零*/
...
putChar(FileInfo *nested, widechar c, TranslationTableHeader **table) {
	 ...
	 * return a pointer to it. 查看某个字符是否在适当的表中。如果没有，插入。不管是哪种情况，
*返回指向它的指针*/
...
putDots(FileInfo *nested, widechar d, TranslationTableHeader **table) {
	 ...
	 * case, return a pointer to it.查看点图案是否在适当的表格中。如果没有，插入。在任一种情况下
* case，返回一个指向它的指针 */
	TranslationTableOffset bucket;
	TranslationTableCharacter *character;
	TranslationTableCharacter *oldchar;
...
/* Look up a character-dots mapping in a display table. 在显示表中查找字符-点映射*/
...
static int
putCharDotsMapping/*放置字符点映射*/(FileInfo *nested, widechar c, widechar d, DisplayTableHeader **table) {
...
getPartName(int actionPart) {/*获取零件名称*/
	return actionPart ? "action" : "test";
}

static int
passFindCharacters(FileInfo *nested, widechar *instructions, int end,/*传递查找字符*/
...
/* The following functions are called by addRule to handle various cases. addRule调用以下函数来处理各种情况。*/

static void
addForwardRuleWithSingleChar(FileInfo *nested, TranslationTableOffset newRuleOffset,/*添加单字符转发规则*/
	...
	// get the character from the table, or if the character is not defined yet, define it从表中获取字符，或者如果字符尚未定义，则定义它
	...
	if (newRule->opcode >= CTO_Pass2 && newRule->opcode <= CTO_Pass4)
		...
		// if the new rule is a character definition rule, set the main definition rule of如果新规则是字符定义规则，则设置的主定义规则
		// this character to it (possibly overwriting previous definition rules)//将此字符添加到它上面(可能会覆盖以前的定义规则)
		// adding the attributes to the character has already been done elsewhere//向字符添加属性已经在其他地方完成
		if (newRule->opcode >= CTO_Space && newRule->opcode < CTO_UpLow)
			character->definitionRule = newRuleOffset;
	}
	// add the new rule to the list of rules associated with this character将新规则添加到与该字符相关联的规则列表中
//如果新规则是字符定义规则，它将被插入到列表的末尾，否则它将被插入到第一个字符定义规则之前
...
static void
addForwardRuleWithMultipleChars(TranslationTableOffset newRuleOffset,/*添加包含多个字符的转发规则*/
		TranslationTableRule *newRule, TranslationTableHeader *table) {
	/* direction = 0 newRule->charslen > 1 */
...
static void
addBackwardRuleWithSingleCell(FileInfo *nested, widechar cell,/*添加带有单个单元格的反向规则*/
	...
	// adding attributes从表中获取单元格，或者如果单元格尚未定义，则定义它(不
//添加属性))
...
static void
addBackwardRuleWithMultipleCells(widechar *cells, int count,/*添加带有多个单元格的反向规则*/
		TranslationTableOffset newRuleOffset, TranslationTableRule *newRule,
		TranslationTableHeader *table) {
	/* direction = 1, newRule->dotslen > 1 */
	TranslationTableRule *currentRule = NULL;
...
static int
addForwardPassRule(TranslationTableOffset newRuleOffset, TranslationTableRule *newRule,/*添加转发通过规则*/
		TranslationTableHeader *table) {
	TranslationTableOffset *currentOffsetPtr;
	TranslationTableRule *currentRule;
...
static int
addBackwardPassRule(TranslationTableOffset newRuleOffset, TranslationTableRule *newRule,/*添加向后通过规则*/
		TranslationTableHeader *table) {
	TranslationTableOffset *currentOffsetPtr;
	TranslationTableRule *currentRule;
	switch (newRule->opcode) {
...
addRule(FileInfo *nested, TranslationTableOpcode opcode, CharsString *ruleChars,
	 ...
	 * chains and chaining both the chars and dots strings 向表中添加一个规则，使用哈希函数查找
*链接字符和点串*/
	...
	/* link new rule into table. 将新规则链接到表中*/
...
findCharacterClass(const CharsString *name, const TranslationTableHeader *table) {/*查找字符类*/
	/* Find a character class, whether predefined or user-defined 查找字符类，无论是预定义的还是用户定义的*/
...
getNextNumberedAttribute(TranslationTableHeader *table) {/*获取下一个编号属性*/
	/* Get the next attribute value for numbered attributes, or 0 if there is no more
	 * space in the table. 获取编号属性的下一个属性值，如果没有，则为0
*表中的空间*/
...
static int
compileRule(FileInfo *nested, TranslationTableOffset *newRuleOffset,/*typedef unsigned int TranslationTableOffset;*/
		TranslationTableRule **newRule, TranslationTableHeader **table,
		DisplayTableHeader **displayTable) {
	int lastToken = 0;//最后一个令牌
	int ok = 1;
	CharsString token;
	/*typedef struct CharsString {
	widechar length;
	...
	TranslationTableOpcode opcode;/*翻译表操作码*/
	CharsString ruleChars;/*typedef struct CharsString {
	                        widechar length;
	                        widechar chars[MAXSTRING];
                            } CharsString;*/
	CharsString ruleDots;
	...
	TranslationTableCharacterAttributes after = 0;/*typedef unsigned long long TranslationTableCharacterAttributes;*/
	...
	widechar uppercase;
	widechar lowercase;
    } TranslationTableCharacter;翻译表字符*/
	widechar *patterns = NULL;
	int k, i;
	int noback, nofor, nocross;
	noback = nofor = nocross = 0;
doOpcode:
	if (!getToken(nested, &token, NULL, &lastToken)) return 1;	/* blank line  空行*/
	if (token.chars[0] == '#' || token.chars[0] == '<') return 1; /* comment   如果这一行开头是#<就直接返回这行不编译*/
	...
	case CTO_Display:
		...
		break;
	/* now only opcodes follow that don't modify the display table 现在只有后面的操作码不修改显示表*/
	default:
		if (!table) break;
		switch (opcode) {
		case CTO_None:
...
compileString(const char *inString, TranslationTableHeader **table,
		DisplayTableHeader **displayTable) {
	/* This function can be used to make changes to tables on the fly.此功能可用于动态更改表格 */
	int k;
	FileInfo nested;
 ...
 */
static char *
resolveSubtable(const char *table, const char *base, const char *searchPath) {/*解析子表*/
	char *tableFile;
	static struct stat info;

	if (table == NULL || table[0] == '\0') return NULL;/*存放表名的数组？*/
	tableFile = (char *)malloc(MAXSTRING * sizeof(char) * 2);//指向一个开辟的字符类型的空间

	//
	// First try to resolve against base 首先尝试解决对基地
	//
	if (base) {
		int k;
	...
	//
	// Then search `LOUIS_TABLEPATH`, `dataPath` and `programPath`然后搜索` LOUIS_TABLEPATH '、` dataPath '和` programPath '
	//
	if (searchPath[0] != '\0') {
		char *dir;
		int last;
 ...
 */
char **EXPORT_CALL
_lou_defaultTableResolver(const char *tableList, const char *base) {/*默认表解析器*/
	char *searchPath;
	char **tableFiles;
	char *subTable;
	char *tableList_copy;
	...
	/* Set up search path 设置搜索路径*/
	searchPath = _lou_getTablePath();/*返回得到一个指向一维数组的指针，数组中包含路劲*/

	/* Count number of subtables in table list 计算表列表中子表的数量*/
	k = 0;
	for (cp = (char *)tableList; *cp != '\0'; cp++)
		if (*cp == ',') k++;
	tableFiles = (char **)calloc(k + 2, sizeof(char *));/*tableFiles开辟空间*/
	if (!tableFiles) _lou_outOfMemory();/*内存不足*/

	/* Resolve subtables解析子表 */
	k = 0;
...
/*函数指针*tableResolver，返回指针的指针，传入参数是 *tableList和*base*，后续就可以用*tableResolver来使用这个函数*/
static char **(EXPORT_CALL *tableResolver)(const char *tableList, const char *base) = &_lou_defaultTableResolver;/*_lou_defaultTableResolver得到return tableFiles*/

static char **
copyStringArray(char **array) {/*复制字符串数组*/
	int len;
	char **copy;
...
char **EXPORT_CALL
_lou_resolveTable(const char *tableList, const char *base) {/*解析表格*/
	char **tableFiles = (*tableResolver)(tableList, base);
	char **result = copyStringArray(tableFiles);/*复制字符串数组返回一个新开辟的指针内容一样*/
	if (tableResolver == &_lou_defaultTableResolver) free_tablefiles(tableFiles);
	return result;/*返回一个指向字符指针的指针，该字符指针指向*/
}

/**
 * Register a new table resolver. Overrides the default resolver.注册一个新的表解析器。覆盖默认冲突解决程序。
 *
 ...
 */
static int
compileFile(const char *fileName, TranslationTableHeader **table,/*编译文件  fileName是存有文件指针的数组*/
		DisplayTableHeader **displayTable) {
	FileInfo nested;
	...
	nested.status = 0;
	nested.lineNumber = 0;
	if ((nested.in = fopen(nested.fileName, "rb"))) {/*打开文件读取文件内容给.in*/
		while (_lou_getALine(&nested))/* Read a line of widechar's from an input file 从输入文件中读取一行宽字符*/
...
/**
 * Implement include opcode实现包含操作码
 *
 */
static int
includeFile(FileInfo *nested, CharsString *includedFile, TranslationTableHeader **table,
		DisplayTableHeader **displayTable) {/*包含文件*/
	int k;/**/
...
/**
 * Compile source tables into a table in memory将源表编译成内存中的表
 *
 */
static int
compileTable(const char *tableList, const char *displayTableList,
	...
	if (translationTable) allocateTranslationTable(NULL, translationTable);/*分配翻译表*/
	...
	/*编译正确操作所必需的东西
    * liblouis或liblouisxml或liblouisutdml */
   /*待办事项:这些定义似乎是正常运行所必需的
    liblouisutdml。找到一种不用硬编码就能满足这些需求的方法
    每个表中的一些字符，特别是用户背后的字符*/
	...
	if (displayTable && translationTable && strcmp(tableList, displayTableList) == 0) {
		/* Compile the display and translation tables in one go 一次性编译显示和翻译表*/

		/* Compile all subtables in the list 编译列表中的所有子表*/
		if (!(tableFiles = _lou_resolveTable(tableList, NULL))) {/*解析表格，tableFiles是指针的指针*/
			errorCount++;
			goto cleanup;
		}
		...
		/* Compile the display and translation tables separately 分别编译显示表和翻译表*/

		if (displayTable) {
			if (!(tableFiles = _lou_resolveTable(displayTableList, NULL))) {/*解析表格,tableFiles是*/
				errorCount++;
				goto cleanup;
			}
			for (subTable = tableFiles; *subTable; subTable++)
		...
		if (translationTable) {
			...
			for (subTable = tableFiles; *subTable; subTable++)/*循环遍历获得的各个表*/
				if (!compileFile(*subTable, translationTable, NULL)) goto cleanup;/*subtable是tableFiles*/
		}
	}

/* Clean up after compiling files 编译文件后清理*/
cleanup:
	free_tablefiles(tableFiles);
...
/* Return the emphasis classes declared in tableList. 返回表列表中声明的强调类*/
...
getTable(const char *translationTableList, const char *displayTableList,
		TranslationTableHeader **translationTable, DisplayTableHeader **displayTable) {
	/* Keep track of which tables have already been compiled 跟踪已经编译了哪些表*/
	...
	/* See if translation table has already been compiled 查看翻译表是否已经编译*/
	if (translationTable) {
		...
		*translationTable = NULL;
		...
		while (currentEntry != NULL) {
			if (translationTableListLen == currentEntry->tableListLength &&
				...
				/* Move the table to the top of the table chain. 将表格移动到表格链的顶部*/
				if (prevEntry != NULL) {
