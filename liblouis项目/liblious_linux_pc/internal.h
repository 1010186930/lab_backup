 ...
 */
typedef struct { /* translation table 转换表*/

	/* state needed during compilation编译期间需要的状态 */
	TranslationTableOffset tableSize;
	TranslationTableOffset bytesUsed;
	...
	TranslationTableCharacterAttributes
			numberedAttributes[8]; /* attributes 0-7 used in match rules (could also be
								   ...
								   faster) 匹配规则中使用的属性0-7(也可以存储在字符类中，但这稍微快一点)*/
	int usesAttributeOrClass;	  /* 1 = attribute, 2 = class */

	/* needed for translation or other api functions翻译或其他api函数需要 */
	int capsNoCont;
	int numPasses;
	int corrections;
	int syllables;
