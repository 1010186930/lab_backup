...
void SUOCUN_init(void)
{
	...
	void DX_ShowBraille(char tbtmp[10][13]){
	 //1.字符串重构，并转整型
	 /*配合IO                                                                                                                的有限没有120个IO口，采用15个锁存器扩展IO口，15*8=120，所以数据必须后期处理8位为一组.									同时考虑到盲文点字序号和stm32的IO输出（硬件连接锁存器D0~D8低到高）顺序相反，所以必须翻转后输出达到正确控制线圈而显示盲文点字*/
