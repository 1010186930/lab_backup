...
//	if(strstr((const char*)path,"韵母")&&!strstr((const char*)path,"韵母"))
//	{
//		Show_Str(18,80,800,32,(u8*)buffer1,32,1);
//		if(strstr(buffer1,"üe"))
//			SPeech("ve1 2 3 4 5 6点");
//		else if(strstr(buffer1,"üan"))
//			SPeech("van1 1 2 3 4 6点");
//		else if(strstr(buffer1,"ün"))
//			SPeech("vn1 4 5 6点");
//		else if(strstr(buffer1,"ü"))
//			SPeech("v1 3 4 6点");
//		else
//			SPeech(buffer1);
//	}
...
但汉字的ASCII比数字和符号的ASCII大。如果有书签10 XXX，最终结果为书签1 XXX,书签10 XXX，书签2 XXX.....*/
{
	...
	if(n==MAXBK)//如果书签数目是10个，就进行如下交换：先交换10和1的顺序，然后依次交换1后面相邻的两个序号，把10交换到最后一位
	{
		strcpy(tmp, pt[0]);
		strcpy(pt[0], pt[1]);
		strcpy(pt[1], tmp);
