...
//W小说文字演示
	void LCD_WShowBraille(u16 x,u16 y,char braille[20][7]){
		char k,l;//K:字序。10个字，0-9；l：字的点序，0-12，12个点表示两方一个字
	int x0,y0;
		for(k=0;k<20;k++)
			for(l=0;l<6;l++)
...
void LCD_ShowBraille(u16 x,u16 y,char braille[10][13]){
			...
			for(l=0;l<12;l++)
	{
		...
		y0=y+150*(k/10)+25*(l%3);     //y 上下相差25
//LCD_Draw_Circle(x+32*(k%5)+8*(l/3)-1*(l/6),y+100*(k/5)+8*(l%3),2*(braille[k][l]-48));
//LCD_Draw_Circle(x+40*(k%10)+8*(l/3)-1*(1/6),y+80*(k/10)+8*(l%3),1*(braille[k][l]-48));
	if(braille[k][l]=='1')
		gui_fill_circle(x0,y0,3,BLUE);
	else LCD_Draw_Circle(x0,y0,2);
}
