...
u8 keyi=1,keyt=1,keyf=1,keys=1,Tkey=1;//菜单下导航标志变量，初始化为指向首行
 ...
 u8 testmenu[7][23]={"声母韵母","音节声调","数字及标符","英文字母"};	
...
if(count_s==9){	
	Show_Str(30,180,800,32,(u8 *)SMbuf,32,1);	
	
		strncat(Sbuf,SMbuf,50);
//	if(pin==1||2||3)
//	 SPeech("[i1]"); 
//	else SPeech("[i2]");
		// SPeech((u8 *)Sbuf);   //调用发声函数，读出指定长度数据 
  ...
  // delay_ms(1);
		...
		memset(Sbuf,'\0',sizeof(Sbuf));
			//LCD_PriOLBraille(28,240,SDbtmp);
